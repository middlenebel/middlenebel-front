...
@Component({
    selector: 'app-action-portforward-list',
    template: `
        <div class="actions">
          <div> 
            <h2 class="title">{{ actionsSelected }}</h2>
            <div *ngIf="description!=undefined">
              <p class="description"> {{ this.description }}</p>
            </div>
            <div class="listing-description">
              <p class="listing-portForward" *ngFor="let itemPF of actionsItem?.portForwards!">
                  {{ itemPF.appName }} - {{ itemPF.port }}
              </p>
            </div>
  ...
  `,
    standalone: true,
    styleUrls: ['./action-portforward-list.component.css'],
    imports: [CommonModule, MatFormFieldModule, MatSelectModule,
        MatInputModule,
        MatButtonModule,
        FormsModule, TestComponent, QuestionComponent]
})
export class ActionPortforwardListComponent {
  @Input() actionsItem?: any; //ElementInterface;
  @Input() actionsSelected? : string;
  @Input() core? : ItemCoreComponent;

  description? : string= "Portforwads defined:";
